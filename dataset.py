...
def build_dataset(paths, labels=None, bsize=32,
    ...
    if tool_gen_mask is not None:
        dset = dset.map(lambda x,y:(tf.py_function(func=tool_gen_mask, inp=[x], Tout=[tf.float32]), y))
        dset = dset.map(lambda x,y:(tf.squeeze(x, 0),y), num_parallel_calls=AUTO)
        
    dset = dset.map(lambda x,y:(clip_image(x),y), num_parallel_calls=AUTO)
...
def build_dataset_from_X_Y(X_path, Y_int, all_class, with_labels, label_mode, img_size,
                           batch_size, repeat, shuffle, augment, im_size_before_crop=None, tool_gen_mask=None):
    ...
    dataset = build_dataset(X_path, Y_int, bsize=batch_size, decode_fn=decoder,
                            repeat=repeat, shuffle=shuffle, augment=augment_img, tool_gen_mask=tool_gen_mask)
